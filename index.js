...
const public_api_key = process.env.PUBLIC_KEY;
const private_api_key = process.env.PRIVATE_KEY;
...
let Coss = () => {
    ...
    async function transmit_encrypted_request(method, url, payload){
        //var payload_json_str = urllib.parse.urlencode(payload) if method == HttpMethod.GET else json.dumps(payload, separators=(',', ':'))
        var payload_json_str = method == HttpMethod.GET ? querystring.stringify(payload) : JSON.stringify(payload)
        var sign = transmit_payload_signature_get(payload_json_str)
        var req_headers = transmit_signed_header_get(sign)
    
        console.log(`sending ${payload_json_str} to ${url}`)
...
//AUTHBUILDER
    function transmit_payload_signature_get(payload_json_str){
        //var sign = hmac.new(self.private_api_key, payload_json_str.encode('utf8'), hashlib.sha256).hexdigest()
        var sign =  CryptoJS.HmacSHA256(payload_json_str, private_api_key).toString(CryptoJS.enc.Hex)
        return sign
    }
